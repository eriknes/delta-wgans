...
class wGAN():
    ...
    def buildDiscriminator(self):
        ...
        discriminator.add(Convolution2D(16, kernel_size=(5,5), strides=(2,2), padding="same"))
        #discriminator.add(ZeroPadding2D(padding=((0,1),(0,1))))
        #discriminator.add(BatchNormalization(momentum=0.7))
        discriminator.add(LeakyReLU(.2))
        discriminator.add(Dropout(0.3))

        discriminator.add(Convolution2D(32, kernel_size=(5,5), strides=(2,2), padding="same"))
        #discriminator.add(ZeroPadding2D(padding=((0,1),(0,1))))
        #discriminator.add(BatchNormalization(momentum=0.7))
        discriminator.add(LeakyReLU(.2))
        discriminator.add(Dropout(0.3))
    ...
    def trainGAN(self, X_train, epochs = 10, batch_size = 64, sample_interval = 1):
        ...
        for epoch in range(epochs + 1):
            ...
            EMdist.append(d_loss[0] + d_loss[1])
            ...
            if epoch % sample_interval == 0:
                ...
                #self.saveEMdist(epoch,EMdist, EMdist_gp)
                self.plotLoss(epoch, dLosses, gLosses, dLosses0, EMdist, EMdist_gp)
    ...
    def plotLoss(self, epoch, dLosses, gLosses, dLosses0, EM, EM_gp):
        ...
        plt.figure(figsize=(10, 8))
        plt.plot(EM, label = 'EM distance')
        plt.xlabel('Epoch')
        plt.ylabel('Wasserstein Cost')
        plt.legend()
    ...
    def saveModels(self, epoch):
        self.generator.save('models/gen_epoch_%d.h5' % epoch)
        self.discriminator.save('models/disc_epoch_%d.h5' % epoch)
