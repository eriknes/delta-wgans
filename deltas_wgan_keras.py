...
class wGAN():
	...
	def buildDiscriminator(self):
		...
		discriminator.add(Conv2D(32, kernel_size=(4,4), strides=2, input_shape=self.dimensions, 
			padding="same", kernel_initializer=initializers.RandomNormal(stddev=0.02)))
		discriminator.add(LeakyReLU())
		discriminator.add(Dropout(0.2))
		discriminator.add(Conv2D(64, kernel_size=(4,4), strides=2, padding="same",
			kernel_initializer=initializers.RandomNormal(stddev=0.02)))
		#discriminator.add(ZeroPadding2D(padding=((0,1),(0,1))))
		discriminator.add(BatchNormalization(momentum=0.75))
		discriminator.add(LeakyReLU())
		discriminator.add(Dropout(0.2))
		discriminator.add(Conv2D(128, kernel_size=(4,4), strides=2, padding="same",
			kernel_initializer=initializers.RandomNormal(stddev=0.02)))
		discriminator.add(BatchNormalization(momentum=0.75))
		discriminator.add(LeakyReLU())
		#discriminator.add(Dropout(0.2))
...
if __name__ == '__main__':
	wgan = wGAN()
	wgan.trainGAN(epochs = 500, batch_size = 64, sample_interval = 10)
